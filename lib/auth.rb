...
module Auth
  ...
  include Common
  include Messages
  @@login_url = 'https://www.wherigo.com/login/'
  @@home_url = 'https://www.wherigo.com/'
  @@user = nil
  # cookie to be sent
  @@cookie = nil
  ...
  def saveCookie(cookie)
    ...
    return if ! cookie
    debug "saveCookie: merge #{hideCookie(cookie)}"
  ...
  def checkLoginScreen(cookie)
    ...
    ### 20131114
    @postVars = Hash.new
    page = ShadowFetch.new(@@home_url + 'home.aspx')
    page.localExpiry = 1
    debug "Checking validity of cookie #{hideCookie(cookie)}"
    data = page.fetch
    data.each_line do |line|
      case line
      # <h3>Welcome back, <span id="ContentPlaceHolder1_lblUsername">Ölscheich99</span>!</h3>
      when /lblUsername\">(.*?)</
        debug "Found greeting as #{$1}"
        puts " ---  " + line.gsub(/<[^>]*>/, '')
        return true
      # <p><strong>Signed In:</strong>
      when /<strong>Signed In:<\/strong>/
  ...
  def getLoginCookie(user, password)
    ...
    @postVars = Hash.new
    # get login form
    debug "create fetch URL #{@@login_url + 'default.aspx'}"
    ...
    page.localExpiry = 0
    @postVars['ctl00$ContentPlaceHolder1$Login1$Login1$UserName'] = user
    @postVars['ctl00$ContentPlaceHolder1$Login1$Login1$Password'] = password
    @postVars['ctl00$ContentPlaceHolder1$Login1$Login1$RememberMe'] = 'on'
    @postVars['ctl00$ContentPlaceHolder1$Login1$Login1$LoginButton'] = 'Sign In'
    page.postVars = @postVars
    page.followRedirect = false
    data = page.fetch
    # extract cookie
    cookie = page.cookie

...
- for every [wid=]cguid, shadowfetch (without prior login!)
...
- finally, log in once (and check)
  cookies: userid, .ASPXAUTH, ASP.NET_SessionId
- for each cguid, (shadow)fetch download page (after login!)
- for each device, (shadow)fetch cartridge by version and output to wid-name-version-device.gwc
